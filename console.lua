-- console.lua (replaced)
-- This file is a lightweight wrapper that loads `console_improved.lua`.
-- Original file preserved as `console.lua.bak`.

local loaded
local ok, mod = pcall(function()
    return require(script.Parent and script.Parent:FindFirstChild('console_improved') and script.Parent.console_improved or nil)
end)
if ok and mod then
    loaded = mod
end
...
if not loaded then
    -- Try loading from workspace or file for development
    local success
    success, loaded = pcall(function()
        local path = 'console_improved.lua'
    ...
    if not success then
        warn('console_improved not found; console functionality disabled')
        loaded = {}
    end
end
