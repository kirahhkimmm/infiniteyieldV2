...
-- mark both compatibility and new brand flags
pcall(function()
	getgenv().IY_LOADED = true
	getgenv().WINKIRA_UNIVERSAL_LOADED = true
end)
...
-- lightweight instance creator (safe-sets properties)
...
-- configuration table exposed for plugins / plugins compatibility
...
-- encapsulate parent ScreenGui creation with exploit compatibility
local function createParentGui()
	if get_hidden_gui or gethui then
		local hiddenUI = get_hidden_gui or gethui
		return new("ScreenGui", {Name = randomString(), Parent = hiddenUI()})
	elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
		local Main = new("ScreenGui", {Name = randomString()})
		pcall(function() syn.protect_gui(Main) end)
		Main.Parent = COREGUI
		return Main
	elseif COREGUI:FindFirstChild('RobloxGui') then
		return COREGUI.RobloxGui
	else
		return new("ScreenGui", {Name = randomString(), Parent = COREGUI})
	end
end

local function setTitle(lbl)
	if not lbl then return end
	lbl.Text = ("%s v%s"):format(WINKIRA_CONFIG.brand or BRAND_NAME, WINKIRA_CONFIG.version or currentVersion)
end
